  ...
  useEffect(() => {
    const initSidebar = async () => {
      try {
        ...
        const filtered = allUsers.filter((u: User) => u.id !== actualId).slice(0, 8);
    ...
    <div className="w-1/4 h-full border-gray-100 flex flex-col bg-white">
      ...
      <div className="p-4 flex items-center justify-between border-b border-gray-50 bg-white">
        ...
        <div className="flex gap-1 text-gray-400">
          <button className="p-2 hover:bg-gray-100 rounded-full transition-colors"><MessageSquarePlus size={18} /></button>
      ...
      {/* Search Bar Section */}
      <div className="p-3">
        <div className={`relative flex items-center rounded-xl transition-all duration-200 ${isSearchFocused ? "ring-2 ring-blue-500/20" : ""}`}>
      ...
      <div className="flex-1 overflow-y-auto">
        {isSearchFocused && !globalSearchTerm ? (
            ...
            {isSearching ? (
              ...
              <>
                ...
                {!globalSearchResults?.rooms?.length && !globalSearchResults?.users?.length && (
                  <div className="p-8 text-center text-gray-400 text-xs italic">No results found for "{globalSearchTerm}"</div>
                )}
              </>
            )}
          </div>
          ...
          rooms?.length ? (
            ...
            <div className="p-8 text-center text-gray-400 text-xs italic mt-10">No chats yet. Try searching for someone!</div>
          )
        )}
      </div>
    </div>
